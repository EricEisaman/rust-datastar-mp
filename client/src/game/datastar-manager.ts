...
export class DatastarUpdateManager {
  ...
  connect(endpoint: string = '/events'): void {
    ...
    this.eventSource.addEventListener('datastar-patch-signals', (event: MessageEvent) => {
      ...
      console.log(`[DatastarManager] 📬 Event data (first 200 chars):`, event.data.substring(0, 200));
  ...
  private handleSignalPatch(data: string): void {
    try {
      ...
      console.log(`[DatastarManager] 📨 Final JSON string to parse (first 200 chars):`, jsonStr.substring(0, 200));
  ...
  private handleElementPatch(data: string): void {
    ...
    try {
      ...
      if (data.startsWith('elements ')) {
        ...
        console.warn(`[DatastarManager] ⚠️ Element patch doesn't start with 'elements ', trying alternative parsing`);
      ...
      if (selector && html) {
        console.log(`[DatastarManager] ✅ Parsed element patch: selector=${selector}, mode=${mode}, html=${html.substring(0, 100)}...`);
        ...
        console.warn(`[DatastarManager] ⚠️ Could not parse element patch - selector: ${selector}, html length: ${html.length}`);
      }
    } catch (err) {
